...
static func create_status(inventoryRef,user_type):
	var sts = Status.new()
...
func activeRegen():
	...
	applyEffect({'metric':-1},general_status.mask_regen)
	...
	if stsConsumable_from_equip.size() > 0:
		for sts in general_status.state.size():
			if stsConsumable_from_equip[sts] != 0:
				applyOnStatus(MetricasDeEstatisticas.createMetricConsumoDeEnergia(null,false),sts,stsConsumable_from_equip[sts])
